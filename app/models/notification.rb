class Notification < ActiveRecord::Base
  attr_accessible :date_time, :notification_type, :death_of, :place, :address, :created_by, :updated_by
  
  # Default Scope
  default_scope where( :disabled => false)

  # Validations
  validates_presence_of :date_time, :notification_type, :death_of, :place, :address, :created_by, :updated_by
  validates_uniqueness_of :death_of, :scope => :notification_type
  validates_inclusion_of :notification_type, in: ["Death", "Condolense"]
  ...
  def validate_date_time
    ...
    if self.notification_type == 'Death'
      unless Notification.where("notification_type = 'Condolense' AND death_of = ? AND date_time < ?", self.death_of, self.date_time).blank?
        return self.errors.add(:base, "Current Date time should be before Date time of Condolense for #{self.death_of}.")
      end
    else
