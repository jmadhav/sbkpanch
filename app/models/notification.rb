class Notification < ActiveRecord::Base
  ...
  validates_inclusion_of :notification_type, in: ["Death", "Final Rights", "Condolence"]
  ...
  def validate_date_time
    ...
    if self.notification_type == 'Death'
      unless Notification.where("notification_type = 'Final Rights' AND death_of = ? AND date_time < ?", self.death_of, self.date_time).blank?
        return self.errors.add(:base, "Current Date time should be before Date time of Final Rights for #{self.death_of}.")
      end
    ...
    elsif self.notification_type == 'Final Rights'
      ...
      unless Notification.where("notification_type = 'Final Rights' AND death_of = ? AND date_time > ?", self.death_of, self.date_time).blank?
        return self.errors.add(:base, "Current Date time should be after Date time of Final Rights for #{self.death_of}.")
      end
      unless Notification.where("notification_type = 'Death' AND death_of = ? AND date_time > ?", self.death_of, self.date_time).blank?
        return self.errors.add(:base, "Current Date time should be after Date time of Death for #{self.death_of}.")
      end
